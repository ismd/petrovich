<? require '_header.php' ?>

<div class="big-image big-image--width-screen">
    <img src="/pictures/culture/recognition.jpg" alt="" />
</div>

<div class="layout">
    <div class="text-content">
        <div class="text-content__page">
            <p>Всегда приятно, когда чувствуешь, что твой вклад видят и&nbsp;ценят. Сразу появляется желание двигаться дальше. &laquo;Петрович&raquo; всегда старается отметить достижения специалистов, поблагодарить за&nbsp;их&nbsp;работу. Например, тем, кто проработал в&nbsp;компании семь лет, вручается премия &laquo;7&nbsp;звезд&raquo;&nbsp;&mdash; очень торжественный и&nbsp;трогательный момент. Каждый год один из&nbsp;специалистов получает награду &laquo;Человек года&raquo;&nbsp;&mdash; за&nbsp;какой-то грандиозный прорыв в&nbsp;работе всей компании. Отдельной награды заслуживают лидеры идей за&nbsp;наибольшее число предложений по&nbsp;улучшению. А&nbsp;для продающих специалистов предусмотрены особые почести&nbsp;&mdash; лучшие из&nbsp;них могут рассчитывать на&nbsp;автомобиль или незабываемое путешествие.</p>
        </div>

        <div class="text-content__center spacing-text">Мероприятия</div>

        <div class="carousel carousel--type-inline">
            <div class="carousel__slide carousel__slide--type-inline">
                <a href="#carousel-1"
                   class="fancybox-fotorama carousel__slide-link">
                    <img src="/pictures/culture/slide-1.jpg" alt="" />
                    <div class="carousel__slide-name">Андрей Иванов</div>
                    <div class="carousel__slide-description">Про развитие в компании</div>
                </a>

                <div class="carousel__hidden">
                    <div id="carousel-1" class="carousel__popup carousel__popup--type-gallery">
                        <div class="spacing-text carousel-popup__title">Наши комитеты</div>

                        <div class="fotorama fotorama--type-mid"
                             data-nav="thumbs"
                             data-auto="false"
                             data-thumbheight="63">
                            <img src="/pictures/culture/photo-1.jpg"
                                 alt="Идеальные условия для того, чтобы ни одна хорошая мысль не потерялась. Премия за улучшения для самых инициативных."
                                 data-caption="Идеальные условия для того, чтобы ни одна хорошая мысль не потерялась. Премия за улучшения для самых инициативных." />
                            <img src="/pictures/culture/photo-2.jpg"
                                 alt="Подпись 2"
                                 data-caption="Подпись 2" />
                        </div>
                    </div>
                </div>
            </div>

            <div class="carousel__slide carousel__slide--type-inline">
                <a href="javascript:void(0)"
                   class="carousel__slide-link">
                    <img src="/pictures/culture/slide-2.jpg" alt="" />
                    <div class="carousel__slide-name">Сергей Петров</div>
                    <div class="carousel__slide-description">Миссия и ценности</div>
                </a>
            </div>

            <div class="carousel__slide carousel__slide--type-inline">
                <a href="javascript:void(0)"
                   class="carousel__slide-link">
                    <img src="/pictures/culture/slide-3.jpg" alt="" />
                    <div class="carousel__slide-name">Александра Сидорова</div>
                    <div class="carousel__slide-description">Направление развития</div>
                </a>
            </div>
        </div>
    </div>
</div>

<? require '_footer.php' ?>
